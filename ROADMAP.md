...
- **`autoindex` resilience.** *Fixed in rc.75* ([#934](https://github.com/xerj-org/xerj/pull/934)): one refused dataset no longer aborts the run, an over-size catalog is split, a throttling node is waited out for 600 s, and `--fresh` adopts old state. Before it: one dataset the server refused aborted the whole run ([#929](https://github.com/xerj-org/xerj/issues/929)), the `--no-graph` progress line is indistinguishable from a hang ([#931](https://github.com/xerj-org/xerj/issues/931)), and `xc-index.sh --fresh` fails on a previously indexed corpus ([#930](https://github.com/xerj-org/xerj/issues/930)). *In flight:* PR [#934](https://github.com/xerj-org/xerj/pull/934) (`fix/autoindex-resilience`). It is on this list because every later item assumes a folder can be indexed unattended.
...
- **A real object-storage backend.** *Today, since rc.75:* `xerj-storage` has a real S3-compatible client (`s3.rs`) and a read-through segment cache, tested against MinIO, with per-request cost accounting by billing class and a budget that stops rather than warns. What is still NOT wired is the thing this item is about: **an index cannot live in a bucket** — nothing on the segment path constructs the backend and `storage.backend = "s3"` still refuses to start on purpose ([#965](https://github.com/xerj-org/xerj/issues/965) would wire it). Separately, `xerj autoindex s3://bucket/prefix` reads documents OUT of a bucket, which is a source, not a home.
