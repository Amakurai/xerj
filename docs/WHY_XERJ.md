...
- **One static binary (61.6 MB, neural embedder included), no JVM.** Sub-second cold start. An agent can spawn its own search engine as a subprocess tool. Operationally: no heap tuning, no GC pauses. (A `--no-default-features` slim build without the neural backend is 58.2 MB — the neural backend accounts for 3.4 MB of the total, so dropping it is rarely worth the loss of capability.)
